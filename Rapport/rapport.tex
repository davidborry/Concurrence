...
Pour l'option \textbf{-t1} où la simulation est gérée par la thread principale, le fonctionnement est assez simple : tant que toutes les personnes n'ont pas atteint la destination, répéter pour chacune d'entre elles l'algorithme de déplacement décrit précédemment. On doit donc mettre à jour le monde et l'ensemble des personnes un certain nombre de fois, ce qui peut faire penser au design d'une boucle de jeu. C'est le but: de cette manière, on peut facilement intégrer la représentation graphique à la simulation en redessinant le terrain à chaque mise à jour. \\
...
Plusieurs choix de synchronisations peuvent être implémentés. Le plus simple serait d'associer un \textbf{mutex}, sémaphore avec un compteur de 1, à la carte entière, mais cette solution serait inneficace et reviendrait à répéter le scénario itératif de \textbf{t0}.
